...
ddm_output_path <- paste0(derivatives_path, "MaximalModel/Models/")
...
diagnostic_plot_path <- paste0(derivatives_path, "MaximalModel/DiagnosticPlots/")
...
write.csv(condition_tracker,
          file = paste0(ddm_output_path, "ConditionTracker.csv"),
          row.names = FALSE)
...
for(i in 1:nrow(condition_tracker)){
  ...
  tryCatch({
    ...
    postdens_plot <- ggdensity(data = ddm_draws,
      ...
      xaxistheme + yaxistheme + blankbgtheme +
      paperfacetxtheme +
